    ...
    <>
      <div className="relative max-w-6xl mx-auto">
        ...
        >
          ...
          {/* Shelves (UPDATED HERE) */}
          {Array.from({ length: shelves }, (_, shelfIndex) => (
            <div key={shelfIndex} className="relative">
              {/* Shelf surface */}
              <div className="flex flex-wrap items-end px-8 py-4 
                bg-gradient-to-b from-amber-900/70 via-amber-800/90 to-amber-900 
                border-b-4 border-amber-700/70 
                shadow-inner relative min-h-[160px] overflow-visible gap-x-2 gap-y-6
              ">
                {letters.map((letter, bookIndex) => (
                  <Book
                    key={letter.id}
                    letter={letter}
